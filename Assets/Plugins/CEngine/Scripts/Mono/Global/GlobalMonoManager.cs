...
namespace CYM
{
    ...
    public sealed class GlobalMonoManager : MonoBehaviour
    {
        ...
        private void Awake()
        {
            ...
            Ins = this;
            Unit = new MonoUpdateData(nameof(Unit), BuildConfig.Ins.UnitJobPerFrame);
            Global = new MonoUpdateData(nameof(Global), BuildConfig.Ins.GlobalJobPerFrame);
            View = new MonoUpdateData(nameof(View), BuildConfig.Ins.ViewJobPerFrame);
            Normal = new MonoUpdateData(nameof(Normal), BuildConfig.Ins.NormalJobPerFrame);
        }
        void Update()
        {
            if (!ToggleUpdate)
