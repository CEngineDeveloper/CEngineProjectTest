...
namespace CYM
{
    public class Builder
    {
        ...
        static Tuple<string, string> GetABNameShader(DLCItem item)
        {
            return new Tuple<string, string>(item.Name + "_" + SysConst.BN_Shader.ToLower(), SysConst.BN_Shader.ToLower());
        }
        #endregion

        #region internal build
        static void _BuildManifest(DLCItem dlcItem)
        {
            dlcItem.PreBuild();
        ...
        static void CollectDependencies(string path)
        {
        ...
        /// 打包共享的资源
        /// </summary>
        static void BuildSharedAssets(DLCItem dlc)
        {
            //打散包的时候需要用到
            ...
            if (shaders.Count > 0)
            {
                Tuple<string, string> bundleName = GetABNameShader(dlc);
                AssetBundleBuild build = new AssetBundleBuild();
                build.assetBundleName = bundleName.Item1;
                build.assetNames = shaders.ToArray();
                Builds.Add(build);
                AddToAllBundles(bundleName, false);
            }
        }
        static BuildTarget GetBuildTarget()
        {
            if (LocalConfig.Ins.Platform == Platform.Windows64)
                return BuildTarget.StandaloneWindows64;
            else if (LocalConfig.Ins.Platform == Platform.Android)
                return BuildTarget.Android;
            else if (LocalConfig.Ins.Platform == Platform.IOS)
        ...
        static void OnPreBuild()
        {
            ...
            AssetDatabase.Refresh();
            LocalConfig.Ins.LastBuildTime = DateTime.Now.ToString("d");
        ...
        {
            //SDK PostBuild
            Type tempSDK = PlatformSDK.GetDistributionType();
