...
namespace CYM.DLC
{
    ...
    {
        #region prop
        private readonly static Dictionary<string,AssetBundle> RawBundles = new Dictionary<string, AssetBundle>();
        private readonly static Dictionary<string, UnityEngine.Object> RawAssets = new Dictionary<string, UnityEngine.Object>();
        private readonly static Dictionary<string, Bundle> Bundles = new Dictionary<string, Bundle>();
        private readonly static Dictionary<string, Asset> Assets = new Dictionary<string, Asset>();
        private readonly static List<Asset> AsyncAssets = new List<Asset>();
        private readonly static List<Asset> clearAsyncAssets = new List<Asset>();
        private readonly static HashSet<AssetBundleManifest> Manifests = new HashSet<AssetBundleManifest>();
        #endregion

        #region pub
        public static Dictionary<string, DLCItem> LoadedDLCItems { get; private set; } = new Dictionary<string, DLCItem>();
        #endregion
        ...
        public static Bundle LoadBundle(string bundleName, string dlc, bool asyncRequest = false,bool isPreload=false)
        {
            if (bundleName.IsInv() || dlc.IsInv())
                return null;
            if (bundleName == SysConst.BN_System)
            {
                CLog.Error($"LoadBundle 无法加载系统资源，请使用SystemRes加载");
                return null;
            }
        ...
        #region load
        public static AssetBundle LoadRawBundle(string DLCName,string name)
        {
            if (!BuildConfig.IsLoaded)
            {
                CLog.Error($"{nameof(BuildConfig)}：没有载入，且必须放在Resources目录下");
                return null;
            }

            string fullName = DLCName + name;
            ...
            string path = Path.Combine(basepath, DLCName) + "/" + DLCName.ToLower() + "_" + name + SysConst.Extention_AssetBundle;
        ...
        public static T LoadRawAsset<T>(string DLCName, string bundleName, string assetName)
            where T : UnityEngine.Object
        {
            string fullAssetsName = bundleName + assetName + DLCName;
            if (RawAssets.ContainsKey(fullAssetsName))
                return RawAssets[fullAssetsName] as T;
            var bundle = LoadRawBundle(DLCName, bundleName);
            if (bundle != null)
            {
        ...
        public static List<Bundle> LoadAllShaderBundle()
        {
            List<Bundle> ret = new List<Bundle>();
            if (LoadedDLCItems.Count == 0)
            {
            ...
            foreach (var dlc in LoadedDLCItems)
            {
                ...
                string realBundle = dlc.Key.ToLower() + "_" + SysConst.BN_Shader.ToLower();
                var bundle = LoadBundle(realBundle, dlc.Key, true, true);
                if (bundle != null && bundle.IsValid())
                {
                    ret.Add(bundle);
