...
namespace CYM
{
    ...
    public partial class BaseGlobal : BaseCoreMono
    {
        ...
        {
            var type = PlatformSDK.GetDistributionType();
            if (type == null)
            {
                return;
            }
            PlatSDKMgr = AddComponent(type) as BasePlatSDKMgr;
            CLog.Cyan($"设置渠道:{PlatformSDK.GetDistributionName()},{PlatSDKMgr}");
        }
        ...
        void OnPostAddComponet(BaseMgr ret)
        {
            ...
            //添加Loader
            if (ret is ILoader loader)
            {
                LoaderMgr.AddCustomLoader(loader);
            }
        }
        #endregion
        ...
        #region is
        public static bool IsAllLoadEnd
        {
            get
            {
                if (
