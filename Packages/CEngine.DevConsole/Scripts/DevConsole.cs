...
namespace CYM.DevConsole{
    public class DevConsole : MonoBehaviour, ISerializationCallbackReceiver {
        ...
        public static Settings settings {
            get {
                if(_settings == null) {
                    ...
                    if (_settings == null)
                    {
                        Debug.LogError("需要创建：Resources/DevConsoleSettings");
                        return null;
                    }
        ...
        public void AddCommands(Command.Command[] commands) {
            commandsManager.Add(commands);
        }

        public void RemoveCommand(Command.Command command) {
            commandsManager.Remove(command);
        }

        public void RemoveCommands(Command.Command[] commands) {
            commandsManager.Remove(commands);
        }

        public void RemoveCommandOverloads(Command.Command command) {
            commandsManager.RemoveOverloads(command);
        }

        public void RemoveCommandOverloads(Command.Command[] commands) {
            commandsManager.RemoveOverloads(commands);
        }

        public bool IsCommandAdded(Command.Command command) {
            return commandsManager.IsCommandAdded(command);
        }

        public bool IsCommandOverloadAdded(Command.Command command) {
            return commandsManager.IsCommandOverloadAdded(command);
        }

        public Command.Command[] GetCommands() {
