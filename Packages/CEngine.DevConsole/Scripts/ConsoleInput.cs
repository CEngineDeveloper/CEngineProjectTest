    ...
    public class ConsoleInput {
        ...
        public void Draw(float positionY) {
            ...
            if (DevConsole.settings.autoCompleteBehaviour != Settings.AutoCompleteBehaviour.Disabled) {
                DrawGenericButton(ref rect, autoCompleteContent, DevConsole.Ins.ToggleAutoComplete);
                rect.x += rect.width;
                rect.x += spacing;
            }
            DrawInput(ref rect);
            rect.x += rect.width;
            rect.x += spacing;
            DrawGenericButton(ref rect, submitContent, DevConsole.Ins.SubmitInputText);
            rect.x += rect.width;
            rect.x += spacing;
            ...
            if (DevConsole.settings.enableHistory)
                DrawGenericButton(ref rect, historyContent, DevConsole.Ins.ToggleHistory);
        }
        ...
        void DrawInput(ref Rect rect) {
            ...
            if(DevConsole.settings.autoCompleteBehaviour == Settings.AutoCompleteBehaviour.Auto && wasEmpty && !string.IsNullOrEmpty(text.Trim())) {
                DevConsole.Ins.autoComplete.Open();
                DevConsole.Ins.history.Close();
            }
            rect.x -= 2;
            rect.width += 4;
