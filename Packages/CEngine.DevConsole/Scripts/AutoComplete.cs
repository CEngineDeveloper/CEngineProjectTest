...
{
    [Serializable]
    public class AutoComplete:Window<int, Command.Command> {
        string lastText = null;
        Command.Command[] allCommands;
        Matcher matcher;
        ...
        public override Command.Command current { get { return allCommands[entries[currentIndex]]; } }
        public bool hasMatches { get { return entries.Count > 0; } }

        protected override void OnInitialized() {
            DevConsole.Ins.commandsManager.onCommandAdded += OnCommandsModified;
            DevConsole.Ins.commandsManager.onCommandRemoved += OnCommandsModified;
            lastText = null;
            needsCommandReload = true;
        }

        void OnCommandsModified(Command.Command commandAddedOrRemoved) {
            needsCommandReload = true;
        }
        ...
        protected override void OnBeginDraw() {
            if(needsCommandReload)
                ReloadCommands();
            if(lastText != DevConsole.Ins.input.text) {
                RebuildCommandsToShow();
                lastText = DevConsole.Ins.input.text;
            }
        }

        void ReloadCommands() {
            allCommands = DevConsole.Ins.GetCommands();
            Clear();
        ...
        void RebuildCommandsToShowThreaded(object jobObject) {
            ...
            int[] matches = matcher.GetMatches(DevConsole.Ins.input.text);
            lock(lockObject) {
                if(job.needed) {
                    Clear();
                    AddRange(matches);
        ...
        void Select(int index) {
            DevConsole.Ins.input.text = allCommands[index].name + " ";
            DevConsole.Ins.input.MoveCursorToEnd();
            Close();
        }
        ...
        class Matcher {
            ...
            public Matcher(Command.Command[] commands) {
