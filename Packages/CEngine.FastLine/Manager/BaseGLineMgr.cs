...
namespace CYM.Line
{
    ...
    public class BaseGLineMgr : BaseGFlowMgr
    {
        ...
        #region life
        protected override string SystemPrefabKey => null;
        ...
        {
            base.OnAllLoadEnd2();
            var obj = Resources.Load<GameObject>(GameConfig.Ins.FastlineRender);
            SystemObj =GameObject.Instantiate(obj);
            if (SystemObj != null)
            {
            ...
            {
                CLog.Error($"{GameConfig.Ins.FastlineRender}为null");
            }
            //创建对象池
            LinePool = new ObjPool<FastLineRenderer>(CreateLineRender);
            if (SystemObj != null)
            {
                LineTemp = SystemObj.GetComponent<FastLineRenderer>();
