...
namespace CYM
{
    public class ArchiveMgr<T> : IArchiveMgr where T : DBBaseGame, new()
    {
        ...
        string GetArchiveOldPath(string name) => Path.Combine(_basePath, name + "_old" + SysConst.Extention_Save);
        // 得到所有文件
        ...
        public ArchiveFile<T> Save(string id, T GameData, bool isAsyn = true,bool isDirtyList=true, bool isHide = false, Callback<bool> callback = null)
        {
            ...
            //存档备份
            var curAchivePath = GetArchivePath(id);
            var oldAchivePath = GetArchiveOldPath(id);
            if (File.Exists(oldAchivePath))
            {
                File.Delete(oldAchivePath);
            }
            FileInfo curfi = new FileInfo(curAchivePath);
            if (curfi.Exists)
            {
                curfi.MoveTo(oldAchivePath);
            }
            //存储
            if (isAsyn)
            {
