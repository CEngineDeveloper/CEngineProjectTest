...
namespace CYM.DebugMenu
{
    public class DebugMenu : MonoBehaviour
    {
        ...
        {
            instance = this;
            config = Resources.Load(nameof(DebugMenuConfiguration)) as DebugMenuConfiguration;

            // Fallback to defaults
            if (config == null)
            {
                CLog.Error($"需要创建：Resources/{nameof(DebugMenuConfiguration)}");
                return;
            }
